...
export function AdminBloodRequestsManager({ token }: AdminBloodRequestsManagerProps) {
  ...
  const handleVerifyRequest = async (requestId: string, isSOS?: boolean) => {
    if (!token) return

    setVerifyingId(requestId)
    try {
      const payload: Record<string, any> = { verified: true }
      if (isSOS) {
        payload.verificationLevel = 1
      }
      ...
      if (response.ok) {
        setSuccessMessage(isSOS ? "SOS Request verified and upgraded to Level 1" : "Blood request accepted")
        fetchRequests()
        setTimeout(() => setSuccessMessage(""), 3000)
      } else {
                    ...
                    >
                      <div className="flex items-start justify-between mb-3">
                        ...
                        <div className="flex flex-col items-end gap-2">
                          ...
                          <Badge className={`${request.verified ? "bg-green-100 text-green-800" : "bg-yellow-100 text-yellow-800"}`}>
                            {request.verified ? "Accepted" : "Awaiting Review"}
                          </Badge>
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <div className="flex flex-wrap gap-2 justify-end">
                          {request.verified !== true && !request.isSOS && (
                            <Button
                            ...
                            >
                              {verifyingId === request._id ? "Accepting..." : "Accept Document"}
                            </Button>
                          )}
                          {request.isSOS && request.verificationLevel === 3 && (
                            <Button
                              onClick={() => handleVerifyRequest(request._id, true)}
                              size="sm"
                              className="bg-red-600 hover:bg-red-700 animate-pulse"
                              disabled={verifyingId === request._id}
                            >
                              {verifyingId === request._id ? "Accepting..." : "Verify SOS"}
                            </Button>
                          )}
                          <Button
