...
import { isRareBloodGroup, getRareGroupDescription } from "@/lib/domain/rare-blood-registry"
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              >
                ...
                <option value="AB-">AB-</option>
                <option value="Bombay (Oh)">Bombay (Oh) — Rare</option>
              </select>

              {/* Rare Blood Group Consent */}
              {formData.bloodGroup && isRareBloodGroup(formData.bloodGroup) && (
                <div className="mt-3 p-3 rounded-lg border border-purple-200 bg-purple-50 space-y-2">
                  <p className="text-xs font-semibold text-purple-900">
                    🩸 Rare Blood Group Registry — {formData.bloodGroup}
                  </p>
                  <p className="text-xs text-purple-800">
                    {getRareGroupDescription(formData.bloodGroup)}
                  </p>
                  <label className="flex items-start gap-2 cursor-pointer">
                    <input
                      type="checkbox"
                      checked={formData.rareGroupConsent}
                      onChange={(e) =>
                        setFormData({ ...formData, rareGroupConsent: e.target.checked })
                      }
                      className="mt-0.5 w-4 h-4 accent-purple-600"
                    />
                    <span className="text-xs text-purple-800">
                      I consent to be contacted any time, including off-hours, for emergency
                      blood requests matching my rare blood group.
                    </span>
                  </label>
                </div>
              )}
            ...
            <div className="p-4 rounded-lg border border-blue-200 bg-blue-50 space-y-3">
              <p className="text-sm font-medium text-blue-900 flex items-center gap-1">
                <MapPin className="w-4 h-4" /> Donor Location (for nearby matching)
              </p>
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1">
                  <label className="text-xs font-medium text-blue-800">City</label>
                  <Input
                ...
                <div className="space-y-1">
                  <label className="text-xs font-medium text-blue-800">Pincode</label>
                  <Input
                ...
                onClick={() => {
                  ...
                  navigator.geolocation.getCurrentPosition(
                    async (pos) => {
                      ...
                      setFormData((p) => ({ ...p, lat: String(latitude), lng: String(longitude) }))
                      // Reverse geocode
                      try {
                        const r = await fetch(`/api/geolocation?lat=${latitude}&lon=${longitude}`)
                        if (r.ok) {
                          const d = await r.json()
                          if (d.locationName) {
                            setFormData((p) => ({ ...p, city: d.locationName }))
                          }
                        }
                      } catch {}
                      setGeoStatus(`✅ Location saved (${latitude.toFixed(4)}, ${longitude.toFixed(4)})`)
                      setIsGeolocating(false)
                    },
                    () => {
                      setGeoStatus("Could not get location. Please enter manually.")
                      setIsGeolocating(false)
                    }
                  )
                }}
              ...
              {geoStatus && (
                <p className="text-xs text-blue-700">{geoStatus}</p>
              )}
              <p className="text-xs text-blue-600/70">
                Your location is used to match you with nearby blood requests. It is never shared publicly.
              </p>
            </div>
            <div className="space-y-2">
